...
本周已经把“平台能跑”和“数据能用”往后推进到了可评估阶段。现在已有一条比较完整的实验链路：
...
| 动作代理接口 | `action_proxy_v0` | 为动作条件 world model 准备第一版动作侧张量 |

这说明当前不是只有一个仿真截图，而是已经形成了“仿真日志 -> 训练样本 -> baseline -> 扰动 -> 置信区间 -> 多 seed -> 跨 seed -> 动作接口”的实验雏形。
...
结论：persistence 很强是因为当前仿真步长是 `0.1s`，预测未来 3 步只对应 `0.3s`。这给后续模型设定了一个明确门槛：新模型必须超过这个强 baseline，才算真正有效。
...
结论：当前紧凑 Ridge residual baseline 跨 seed 泛化不好，尤其是链路速率预测明显退化。这不是失败，而是把问题暴露出来了：只靠聚合特征和线性 residual 不够，后续需要更结构化的双图编码和动作条件 world model。

## 7. 动作代理接口 v0

为了往动作条件 world model 推进，已经从可观测日志中构造第一版动作代理变量。

| 字段 | 形状 | 含义 |
|---|---:|---|
| `a_hist` | `(950, 8, 13)` | 与历史输入窗口对齐的动作代理变量 |
| `a_future` | `(950, 3, 13)` | 与未来标签窗口对齐的动作代理变量 |

动作代理特征包括：

- 卸载决策数量和卸载目标类型数量。
- 活跃通信链路数量、RB 分配总量、每条活跃链路平均 RB。
- CPU 计算进度代理和 computing 任务数量。
- UAV 平均速度和平均位移。

需要注意：这只是代理变量，不是严格动作日志。严格版本需要在调度器执行时直接记录每个任务的卸载目标、RB 编号、CPU 分配量和 UAV 移动指令。

## 8. 当前创新点表达

不要说 AirFogSim 是创新点。更准确的表达是：

AirFogSim 提供可控仿真环境。当前工作的重点是把仿真日志组织成适合世界模型学习的联合状态序列，包含物理图、通信图和任务状态，并进一步建立预测、扰动评估、置信区间、跨 seed 泛化和动作条件接口。后续在这个基础上训练动作条件 world model，用于多步状态推演和多场景预训练迁移。

可以拆成三层：

- 数据组织：把原始 `node/link/task` 日志整理成联合时间序列。
- 模型接口：从历史窗口预测未来标签，并预留动作变量接口。
- 评估体系：不只看预测误差，还看扰动稳定性、置信区间、在线耗时和跨 seed 泛化。

## 9. 还没有完成的部分

当前还没有完成正式 world model 训练，也没有完成严格动作日志导出。

下一步优先级：

1. 把动作代理升级为严格调度动作日志。
2. 在多 seed 数据上做扰动训练，而不只是 clean 训练、noisy 测试。
