...
| 多 seed 数据集 | `dataset_multiseed_v0/` | 已经把 5 条随机轨迹整理成 950 个可训练样本 |
...
## 8. dataset_multiseed_v0 现在有什么

多 seed 随机性分析已经进一步整理成可训练数据集：
...
这里每个 seed 产生 190 个样本，总共 5 个 seed，所以样本数从单 seed 的 190 增加到 950。这个数据集的意义是，后面可以做跨 seed 训练测试，例如用 seed 0-3 训练、seed 4 测试，验证模型是否学到可迁移规律。
...
1. 做跨 seed baseline：用 seed 0-3 训练、seed 4 测试，看模型是否能跨随机轨迹泛化。
...
5. 准备 PPT：重点展示 baseline、扰动曲线、置信区间、状态转移机制、计时实验、多 seed 数据集和下一步计划。
