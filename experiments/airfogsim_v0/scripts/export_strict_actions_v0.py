    ...
    def scheduleComputing(self, env):
        def alloc_cpu_callback(computing_tasks, **kwargs):
            ...
            for task_dict in task_list:
                task_id = task_dict["task_id"]
                assigned_node_id = task_dict["assigned_to"]
                fog_profile = (assigned_node_info or {}).get("fog_profile", {})
                alloc_cpu = fog_profile.get("cpu", 0) / max(1, appointed_fog_node_dict[assigned_node_id])
                ...
                self.cpu_rows.append(
                    {
                        ...
                        "num_tasks_on_node": appointed_fog_node_dict[assigned_node_id],
                    }
                )
