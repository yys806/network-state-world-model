...
在不破坏PI-JWM运行入口、测试发现、证据链和历史可追溯性的前提下，将知识记录、代码、论文、文献、组会与杂项文档重构为根目录下清晰稳定的职责目录，并把散落测试与临时材料归入`code/`或`docs/`。

| Phase | Status | Deliverable |
|---|---|---|
| 1. 当前目录与引用审计 | in_progress | 根目录/文档/代码树、文件类型、体积、Git状态、路径引用和运行入口清单 |
| 2. 归类边界确认 | in_progress | 已确认paper只放正式论文材料；其余根文件与历史artifact边界在设计中冻结 |
| 3. 迁移设计与方案比较 | complete | 用户选择分阶段原子迁移；旧路径不保留长期别名，历史manifest通过兼容解析验收 |
| 4. 用户确认与成文规格 | complete | 用户确认书面规格；设计提交`0152548` |
| 5. 实施计划 | in_progress | 用户已确认verify-only验收修订；实施计划写入`记录/实施计划/2026-08-16-仓库顶层目录重构实施计划.md`并进入自审 |

### Fixed Boundaries

- 设计确认前只读审计，不移动或重命名文件。
- `AGENTS.md`必须继续位于Git工作树根，确保后续任务自动加载项目规则；根`README.md`是否保留由设计明确。
- 历史机器artifact中的旧绝对路径和哈希绑定不能静默改写；若迁移使其不可执行，必须明确兼容/归档边界。
- `D:\shen\PKU\RRM`始终在本轮范围外。

### Errors Encountered

| Error | Attempt | Resolution |
|---|---:|---|
| 设计确认补丁遇到规划表重复的Phase 4行，上下文不匹配 | 1 | 无写入；重新读取真实表头，合并为单一Phase 4后继续 |
...
核验并归档`文档/文献/待整理/`中的手动下载PDF，按本地权威索引更新分类、路径、哈希和状态文件；仅在精确确认不再被本地文献库依赖后清除PIJWM的Zotero遗留文件与表述。

| Phase | Status | Deliverable |
|---|---|---|
| 1. 基线与待整理清点 | complete | 14个PDF、现有64项索引基线和Zotero遗留候选已清点 |
| 2. 文献身份匹配与去重 | complete | 13篇正式版逐项DOI匹配；1篇Conformal arXiv版本单列；14个新哈希零重复 |
| 3. 分类归档与索引重建 | complete | 14个PDF已移动，CSV/Markdown/状态/哈希和缺失清单已同步到78篇全覆盖 |
| 4. Zotero遗留清理 | complete | 8个一次性导出/附件/迁移文件已送入回收站；现行本地索引保留 |
| 5. 独立验收 | complete | 78 PDF/78哈希/78索引/78校验行逐项一致，0缺失、0遗留命名路径、验收0错误 |

### Fixed Boundaries

- 不触碰独立的`D:\shen\PKU\RRM`项目及其文献。
- 不把历史Zotero附件标志当作PDF存在证据；以本地PDF、SHA-256和索引路径为准。
- 不覆盖同名文件；先用DOI、题名、作者/年份和哈希确认身份，再移动。
- `docs/superpowers/`现有tracked deletions属于既有工作树状态，本轮不恢复、不扩大。

### Errors Encountered

| Error | Attempt | Resolution |
|---|---:|---|
| PowerShell移动预检把`foreach`语句直接连接到格式化管道，解析报`An empty pipe element is not allowed` | 1 | 命令在解析阶段失败、无文件变化；改为用数组收集结果后再输出，不重复原语法 |
| 遗留文件统计的另一段`foreach`仍直接接格式化管道，触发同类解析错误 | 2 | 无文件变化；后续命令所有循环均先写入显式数组变量，完成后再输出，杜绝第三次 |
| 移动命令中的父目录附加检查使用当前PowerShell无法解析的`Split-Path -LiteralPath ... -Parent`参数组合，产生14次非终止错误 | 1 | 根路径、来源唯一、目标不存在检查已通过且14项移动完成；不重复原检查，改用`[IO.Path]::GetDirectoryName()`独立复核目标 |
