...
冻结 Tensor 的最后一个 History 时刻现已可确定性生成严格分离的 Physical / Information typed graph objects：Physical 只含空间位置、运动与空间关系；Information 由 Agent、Task、Comm、Task-Agent、logical Flow、DAG 组成；Carrying 是 Flow side state。跨图只 materialize 同实体 Align 和 wireless Comm endpoint 的 GeoComm dependency。当前 topology 参数仅用于 development validation，并未冻结为 PI-JWM 最终科研选择。Graph Encoder、GNN、World Model、Loss、Planner 与训练均未开始。
