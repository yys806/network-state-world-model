...
\clearpage
\section*{记录说明}
本文档按时间顺序整理 \pjwm{} 已经发生的模型版本推进、实验结果、失败路线与证据边界，用于回答“项目做过什么、为何演进到当前状态”。它是历史记录，不代表其中每个旧版本的定义仍是当前定稿，也不承担论文正文功能。早期 v0--v5 的日期按本地历史报告文件记录日期填写，v6 以后按实验目录、报告或交接文件中的日期填写。表中 $\uparrow$ 表示越大越好，$\downarrow$ 表示越小越好；粗体表示该表内该指标最优。AirFogSim/SUMO 只作为参考仿真器和数据来源，主框架始终为 \pjwm{}。
\section{版本进展总览}
...
\section{v11 selector 阶段条件收益诊断（2026-07-20）}

本周在不重新训练 world model、且不打开 matched test 18--19 与 external holdout 60--69 的前提下，完成了 v11 selector 的根因归因和一个可复核的阶段条件收益基线。此前 CandidateSet ranker 在 validation 上 100\% defer；进一步的 rank-only 归因显示，问题不是回退阈值过于保守，而是候选排序本身失效：rank-only 策略的 validation RMSE 为 292.90--312.33，且所有 validation seed 均恶化。schema-v6 交互审计则显示 opportunity detection 具有信号（ROC-AUC 0.8752、PR-AUC 0.9495），但候选收益排序仍弱（sample rank Spearman 0.0832）。因此本周将“是否存在可修复机会”和“修复后收益有多大”拆开建模，并检验可观测 episode phase 是否提供稳定上下文。

对每个候选 $c$ 和每个 episode local step $p$，仅使用 train seeds 估计 raw-SSE benefit 的均值、跨 seed 正向比例、方差和支持数；episode phase 由 $\text{sample\_id}\bmod390$ 恢复，不使用 seed 身份或 future rollout outcome。calibration seeds 44--49 只用于确定安全执行门槛，validation seeds 50--59 只用于一次性评估。最终采用的 phase-conditioned benefit LCB 规则为：正向比例不低于 0.575、均值收益不低于 $0.0003528595$、支持数至少为 5，并通过可观测 task--energy proxy 的 Pareto 非支配约束；未通过时回退到 ranked-allocation baseline。
...
\bottomrule
\end{tabular}
\caption{v11 阶段条件收益 LCB 在 validation seeds 50--59 上的结果。该表是 \code{diagnostic\_only}，不是独立测试或最终部署结果。}
\label{tab:phase_selector_progress}
\end{table}

该结果达到预注册的 B 级区间 $[200,213.160874)$，但没有达到 A 级 RMSE $<200$，因此 selector 配置仍未冻结，external holdout 仍保持锁定。结果的解释边界也已经明确：phase context 和训练期的跨 seed benefit 统计足以产生稳定的 validation 改善，但不能据此宣称 matched test 泛化。candidate-specific experts、phase smoothing、estimator ensemble、learned residual routing、phase-restricted kNN 和无 phase token ranker 均未超过该 exact phase table，作为失败路线保留在审计记录中。

本周同时完成了 GPU 训练前的工程闭环：RTX 4090 上的全数据 CUDA smoke 和 3-seed token probe 均正常，正式 phase selector runner 在本地复现 validation RMSE 207.5398777，冻结摘要 digest 为 \code{887331b2...454a2}；主测试 722/722、脚本测试 84/84、compileall 和 diff audit 均通过。下一步进入 GPU 的前置条件不是继续扩大同一排序器，而是先完成候选收益表示与 selector 结构的独立设计，并在 validation 上达到预设门槛后再一次性打开外部 holdout。完整复现命令、decision trace、per-seed CSV、NPZ 统计缓存和 SHA-256 清单位于“代码”目录下的 \path{artifacts/reports/pi_jwm_v11_selector_finalization_20260719/phase_selector_formal_20260720/}。
