...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_event_memory_activity_routing_20260616"
mkdir -p "$OUT"
...
run_one e0_hurdle_no_memory
run_one e1_event_memory_shared --use-event-memory-features --event-memory-routing shared
run_one e2_event_memory_activity_only --use-event-memory-features --event-memory-routing activity_only
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_event_memory_activity_routing_20260616")
anchor = {
    "experiment": "c8a_hurdle_anchor",
    "event_memory_routing": "none_previous",
    "best_epoch": 25,
...
rows = [anchor]
for name in ["e0_hurdle_no_memory", "e1_event_memory_shared", "e2_event_memory_activity_only"]:
    ...
    row = {
        "experiment": name,
        "use_event_memory_features": cfg.get("use_event_memory_features"),
        "event_memory_routing": cfg.get("event_memory_routing"),
        "activity_memory_dim": cfg.get("activity_memory_dim"),
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "event_memory_activity_routing_comparison.csv").open("w", newline="") as f:
...
with (out / "event_memory_activity_routing_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 Event-Memory Activity Routing GPU Grid\n\n")
    f.write("| Experiment | routing | best epoch | P | R | F1 | Active-rate RMSE | Positive-rate active RMSE | Link RMSE | Node RMSE | Task RMSE | d active | d positive | d link | d F1 |\n")
    f.write("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        f.write("| " + " | ".join([
            fmt(row.get("experiment")),
            fmt(row.get("event_memory_routing")),
...
print(out / "event_memory_activity_routing_comparison.csv")
PY
