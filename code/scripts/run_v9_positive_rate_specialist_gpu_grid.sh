...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_positive_rate_specialist_20260616"
mkdir -p "$OUT"
...
run_one p0_norm_mse --positive-rate-specialist-weight 0.7 --positive-rate-target-mode normalized --positive-rate-loss-mode mse
run_one p1_norm_huber --positive-rate-specialist-weight 0.7 --positive-rate-target-mode normalized --positive-rate-loss-mode huber
run_one p2_log1p_norm_mse --positive-rate-specialist-weight 0.7 --positive-rate-target-mode log1p_normalized --positive-rate-loss-mode mse
run_one p3_log1p_norm_huber --positive-rate-specialist-weight 0.7 --positive-rate-target-mode log1p_normalized --positive-rate-loss-mode huber
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_positive_rate_specialist_20260616")
anchor = {
...
rows = [anchor]
for name in ["p0_norm_mse", "p1_norm_huber", "p2_log1p_norm_mse", "p3_log1p_norm_huber"]:
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "positive_rate_specialist_comparison.csv").open("w", newline="") as f:
...
with (out / "positive_rate_specialist_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 Positive-Rate Specialist GPU Grid\n\n")
    f.write("| Experiment | target | loss | best epoch | P | R | F1 | Active-rate RMSE | Positive-rate active RMSE | Link RMSE | Node RMSE | Task RMSE | d active | d positive | d link | d F1 |\n")
    f.write("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        f.write("| " + " | ".join([
            fmt(row.get("experiment")),
            fmt(row.get("positive_rate_target_mode")),
            fmt(row.get("positive_rate_loss_mode")),
...
print(out / "positive_rate_specialist_comparison.csv")
PY
