...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_high_rate_link_balance_20260617"
mkdir -p "$OUT"
...
COMMON=(
  ...
  --rate-output-mode main --rate-loss-mode active_mixed
...
run_one b0_w2_inactive010 --high-rate-weight 2.0 --inactive-rate-weight 0.10
run_one b1_w2_inactive020 --high-rate-weight 2.0 --inactive-rate-weight 0.20
run_one b2_w4_inactive010 --high-rate-weight 4.0 --inactive-rate-weight 0.10
run_one b3_w4_inactive020 --high-rate-weight 4.0 --inactive-rate-weight 0.20
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_high_rate_link_balance_20260617")
anchor = {
...
high_rate_grid = {
    "h1_high_w2": {
        "active_rate_rmse": 288.146117,
        "link_rate_rmse": 90.890884,
        "f1": 0.014691,
    },
    "h2_high_w4": {
        "active_rate_rmse": 286.921388,
        "link_rate_rmse": 107.837914,
        "f1": 0.022502,
    },
}
rows = [anchor]
names = ["b0_w2_inactive010", "b1_w2_inactive020", "b2_w4_inactive010", "b3_w4_inactive020"]
for name in names:
    ...
    row = {
        ...
        "inactive_rate_weight": cfg.get("inactive_rate_weight"),
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "high_rate_link_balance_comparison.csv").open("w", newline="") as f:
...
with (out / "high_rate_link_balance_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 High-Rate Link-Balance GPU Grid\n\n")
    f.write("Reference high-rate-only results: h1 w=2 active=288.146117/link=90.890884/F1=0.014691; h2 w=4 active=286.921388/link=107.837914/F1=0.022502.\n\n")
    f.write("| Experiment | high w | inactive w | best epoch | P | R | F1 | Active-rate RMSE | Positive-rate active RMSE | Link RMSE | Node RMSE | Task RMSE | d active C8a | d positive C8a | d link C8a | d F1 C8a |\n")
    f.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        f.write("| " + " | ".join([
            fmt(row.get("experiment")),
            fmt(row.get("high_rate_weight")),
            fmt(row.get("inactive_rate_weight")),
...
print(out / "high_rate_link_balance_comparison.csv")
PY
