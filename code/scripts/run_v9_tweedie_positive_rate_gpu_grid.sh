...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_tweedie_positive_rate_20260617"
mkdir -p "$OUT"
...
run_one t0_tweedie_w01_p13 --positive-rate-specialist-weight 0.1 --positive-rate-target-mode raw --positive-rate-loss-mode tweedie --positive-rate-tweedie-power 1.3
run_one t1_tweedie_w01_p15 --positive-rate-specialist-weight 0.1 --positive-rate-target-mode raw --positive-rate-loss-mode tweedie --positive-rate-tweedie-power 1.5
run_one t2_tweedie_w03_p13 --positive-rate-specialist-weight 0.3 --positive-rate-target-mode raw --positive-rate-loss-mode tweedie --positive-rate-tweedie-power 1.3
run_one t3_tweedie_w03_p15 --positive-rate-specialist-weight 0.3 --positive-rate-target-mode raw --positive-rate-loss-mode tweedie --positive-rate-tweedie-power 1.5
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_tweedie_positive_rate_20260617")
anchor = {
    "experiment": "c8a_hurdle_anchor",
    "positive_rate_specialist_weight": 0.0,
    "positive_rate_loss_mode": "none",
    "positive_rate_tweedie_power": "",
    "best_epoch": 25,
...
rows = [anchor]
names = ["t0_tweedie_w01_p13", "t1_tweedie_w01_p15", "t2_tweedie_w03_p13", "t3_tweedie_w03_p15"]
for name in names:
    ...
    row = {
        ...
        "positive_rate_target_mode": cfg.get("positive_rate_target_mode"),
        "positive_rate_loss_mode": cfg.get("positive_rate_loss_mode"),
        "positive_rate_tweedie_power": cfg.get("positive_rate_tweedie_power"),
...
for row in rows[1:]:
    ...
    row["positive_rate_active_delta_vs_anchor"] = row["positive_rate_active_rmse"] - anchor["positive_rate_active_rmse"]
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "tweedie_positive_rate_comparison.csv").open("w", newline="") as f:
...
with (out / "tweedie_positive_rate_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 Tweedie Positive-Rate GPU Grid\n\n")
    f.write("| Experiment | weight | power | best epoch | P | R | F1 | Active-rate RMSE | Positive-rate active RMSE | Link RMSE | Node RMSE | Task RMSE | d active | d positive | d link | d F1 |\n")
    f.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        f.write("| " + " | ".join([
            fmt(row.get("experiment")),
            fmt(row.get("positive_rate_specialist_weight")),
            fmt(row.get("positive_rate_tweedie_power")),
            ...
            fmt(row.get("positive_rate_active_delta_vs_anchor")),
            fmt(row.get("link_rate_delta_vs_anchor")),
            fmt(row.get("f1_delta_vs_anchor")),
        ]) + " |\n")
print(out / "tweedie_positive_rate_comparison.csv")
PY
