...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_predicted_w1_repro_20260617"
mkdir -p "$OUT"
...
run_one p0_seed20260615 --seed 20260615
run_one p1_seed20260617 --seed 20260617
run_one p2_seed20260619 --seed 20260619
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_predicted_w1_repro_20260617")
anchor = {
...
rows = [anchor]
names = ["p0_seed20260615", "p1_seed20260617", "p2_seed20260619"]
for name in names:
...
valid = [row for row in rows[1:] if "error" not in row]
if valid:
    rows.append({
        "experiment": "mean_repro",
        "precision": sum(row["precision"] for row in valid) / len(valid),
        "recall": sum(row["recall"] for row in valid) / len(valid),
        "f1": sum(row["f1"] for row in valid) / len(valid),
        "active_rate_rmse": sum(row["active_rate_rmse"] for row in valid) / len(valid),
        "positive_rate_active_rmse": sum(row["positive_rate_active_rmse"] for row in valid) / len(valid),
        "link_rate_rmse": sum(row["link_rate_rmse"] for row in valid) / len(valid),
        "node_rmse": sum(row["node_rmse"] for row in valid) / len(valid),
        "task_rmse": sum(row["task_rmse"] for row in valid) / len(valid),
    })
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "predicted_w1_repro_comparison.csv").open("w", newline="") as f:
...
with (out / "predicted_w1_repro_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 Predicted W1 Repro GPU Grid\n\n")
    f.write("| Experiment | seed | best epoch | P | R | F1 | Active-rate RMSE | Positive-rate active RMSE | Link RMSE | Node RMSE | Task RMSE | d active | d link | d F1 |\n")
    f.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        f.write("| " + " | ".join([
...
print(out / "predicted_w1_repro_comparison.csv")
PY
