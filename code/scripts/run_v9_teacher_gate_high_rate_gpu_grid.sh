...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_teacher_gate_high_rate_20260617"
mkdir -p "$OUT"
...
run_one t0_predicted_w1 --hurdle-train-gate-mode predicted --high-rate-weight 1.0
run_one t1_teacher_w1 --hurdle-train-gate-mode teacher_forcing --high-rate-weight 1.0
run_one t2_teacher_w2 --hurdle-train-gate-mode teacher_forcing --high-rate-weight 2.0
run_one t3_teacher_w4 --hurdle-train-gate-mode teacher_forcing --high-rate-weight 4.0
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_teacher_gate_high_rate_20260617")
anchor = {
...
rows = [anchor]
names = ["t0_predicted_w1", "t1_teacher_w1", "t2_teacher_w2", "t3_teacher_w4"]
for name in names:
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "teacher_gate_high_rate_comparison.csv").open("w", newline="") as f:
...
with (out / "teacher_gate_high_rate_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 Teacher-Gate High-Rate GPU Grid\n\n")
    f.write("| Experiment | gate | high w | best epoch | P | R | F1 | Active-rate RMSE | Positive-rate active RMSE | Link RMSE | Node RMSE | Task RMSE | d active | d positive | d link | d F1 |\n")
    f.write("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        f.write("| " + " | ".join([
...
print(out / "teacher_gate_high_rate_comparison.csv")
PY
