...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_dynamic_hard_negative_20260617"
mkdir -p "$OUT"
...
run_one d0_hurdle_anchor_repeat --dynamic-hard-negative-weight 0.0
run_one d1_hn_w05_r10 --dynamic-hard-negative-weight 0.5 --dynamic-hard-negative-ratio 0.10
run_one d2_hn_w10_r05 --dynamic-hard-negative-weight 1.0 --dynamic-hard-negative-ratio 0.05
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_dynamic_hard_negative_20260617")
anchor = {
    "experiment": "c8a_hurdle_anchor",
    "dynamic_hard_negative_weight": 0.0,
    "dynamic_hard_negative_ratio": "",
    "best_epoch": 25,
...
rows = [anchor]
for name in ["d0_hurdle_anchor_repeat", "d1_hn_w05_r10", "d2_hn_w10_r05"]:
    ...
    row = {
        "experiment": name,
        "dynamic_hard_negative_weight": cfg.get("dynamic_hard_negative_weight"),
        "dynamic_hard_negative_ratio": cfg.get("dynamic_hard_negative_ratio"),
...
for row in rows[1:]:
    ...
    row["precision_delta_vs_c8a"] = row["precision"] - anchor["precision"]
    row["recall_delta_vs_c8a"] = row["recall"] - anchor["recall"]
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "dynamic_hard_negative_comparison.csv").open("w", newline="") as f:
...
with (out / "dynamic_hard_negative_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 Dynamic Hard Negative GPU Grid\n\n")
    f.write("| Experiment | hn weight | hn ratio | best epoch | P | R | F1 | Active-rate RMSE | Positive-rate active RMSE | Link RMSE | Node RMSE | Task RMSE | d active | d positive | d link | d F1 | d P | d R |\n")
    f.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        f.write("| " + " | ".join([
            fmt(row.get("experiment")),
            fmt(row.get("dynamic_hard_negative_weight")),
            fmt(row.get("dynamic_hard_negative_ratio")),
            ...
            fmt(row.get("precision_delta_vs_c8a")),
            fmt(row.get("recall_delta_vs_c8a")),
        ]) + " |\n")
print(out / "dynamic_hard_negative_comparison.csv")
PY
