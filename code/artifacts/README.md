...
- `literature/`、`research_notes/`：研究过程中形成的检索过程与阅读材料；正式本地PDF和权威索引统一维护在仓库根级`literature/`。
...
实验结论必须区分`deployable`、`true_future_reference`、`sample_oracle`和`test_best_diagnostic`。真实未来动作、逐样本oracle与测试集事后最优不能写成自主可部署结果。当前理论和阶段边界以`记录/`中的最新handoff、`本地计划表.md`和对应机器manifest共同为准。
