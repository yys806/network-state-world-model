...
SCHEMA_VERSION = "PI-JWM-Step-3.3-Model-Input-Tensor-Collation-v1"
...
TENSOR_INPUT_GAP_TABLE = (
    {"field":"channel_rows.feature_values", "status":"endpoint_ids_only; numeric feature not exposed", "follow_up":"03/04 mapping"},
    {"field":"node_cpu_capacity_per_s", "status":"missing_or_masked; not synthesized", "follow_up":"record source/mask when sample exposes it"},
    {"field":"comp.allocated_cpu", "status":"no comp entries in current development bundle", "follow_up":"extend Raw/sample only with authorization"},
    {"field":"raw_simulator_acceleration_mps2", "status":"audit_only", "follow_up":"excluded from model feature"},
    {"field":"future_task_schedule", "status":"metadata_only", "follow_up":"excluded from input namespace"},
)
...
class TensorContract:
    ...
    lifecycle_vocab:tuple[str,...]=( "<PAD>", "unknown")
    entity_type_vocab:tuple[str,...]=( "<PAD>", "unknown")
    route_kind_vocab:tuple[str,...]=( "<PAD>", "unknown")
    transport_vocab:tuple[str,...]=( "<PAD>", "unknown")
...
def _contract(samples,stats,contract):
    ...
    o=_maxima(samples)
    if contract is None:
        contract=TensorContract(history_steps=len(samples[0]["history"]),horizon_steps=len(samples[0]["future_action"]),lifecycle_vocab=_labels(samples,"lifecycle","lifecycle"),entity_type_vocab=_labels(samples,"entity_type","entity"),route_kind_vocab=_labels(samples,"route_kind","route"),transport_vocab=_labels(samples,"transport","flow"),feature_units=tuple(sorted((str(k),str(v.get("unit"))) for k,v in (stats or {}).get("features",{}).items() if v.get("unit"))),**o)
    for k,v in o.items():
...
def build_tensor_batch(samples:Sequence[Mapping[str,Any]],*,stats=None,contract=None):
    ...
    out.update({"target_entity_presence":z((B,L,c.max_target_entity),bool),"target_task_presence":z((B,L,c.max_target_task),bool),"target_flow_presence":z((B,L,c.max_target_flow),bool),"target_task_features":z((B,L,c.max_target_task,1)),"target_task_feature_mask":z((B,L,c.max_target_task,1),bool),"target_task_lifecycle_index":z((B,L,c.max_target_task),np.int64)})
    for bi,s in enumerate(samples):
        idx={k:{str(a):int(v) for a,v in d.items()} for k,d in s["static"]["input_entity_index"].items()}; tgt={k:{str(a):int(v) for a,v in d.items()} for k,d in s["static"].get("target_index",{}).items()}; out["sample_ids"].append(str(s["metadata"]["sample_id"])); out["sample_static"].append({"input_entity_index":copy.deepcopy(idx),"target_index":copy.deepcopy(tgt),"target_only_objects":copy.deepcopy(s["static"].get("target_only_objects",{}))}); out["sample_metadata"].append({"sample_id":str(s["metadata"]["sample_id"]),"trajectory_id":str(s["metadata"]["trajectory_id"]),"split":str(s["metadata"]["split"])})
        for hi,fm in enumerate(s["history"]):
            ...
            if hi<H-1: _actions(out,s["history"][hi].get("action",{}),bi,hi,True,idx,c)
        for li,a in enumerate(s["future_action"]): _actions(out,a,bi,li,False,idx,c)
        for li,tg in enumerate(s["target"]):
            for r in tg.get("entities",[]): out["target_entity_presence"][bi,li,int(r["target_index"])]=bool(r.get("presence",False))
            ...
            for r in tg.get("flows",[]): out["target_flow_presence"][bi,li,int(r.get("target_index",r.get("flow_index",-1)))]=True
    return out
    ...
    for fi,fam in enumerate(ACTION_FAMILIES):
        ...
        for ai,e in enumerate(rec.get("entries",[])):
            ...
            if fam=="route":
                ...
                if e.get("allocated_cpu") is not None: out[f"{p}_comp_allocated_cpu"][bi,ti,ai]=float(e["allocated_cpu"]); out[f"{p}_comp_allocated_cpu_mask"][bi,ti,ai]=True
            else:
...
def validate_tensor_batch(tensor):
    ...
    return bool(tensor["entity_features"].shape==(b,c.history_steps,c.max_entity,2) and tensor["future_action_present"].shape==(b,c.horizon_steps,4) and len(tensor.get("sample_static",[]))==b)
...
__all__=["ACTION_FAMILIES","SCHEMA_VERSION","TENSOR_INPUT_GAP_TABLE","TensorContract","build_tensor_batch","load_tensor_batch","save_tensor_batch","validate_tensor_batch"]
