...
REQUIRED_ACCEPTANCE_CHECKS = frozenset(ORIGINAL_ACCEPTANCE_CHECKS + ADDITIONAL_SERVICE_CHECKS)
...
class StructuredRSSMWorldModel(nn.Module):
    ...
    def __init__(self, config: StructuredRSSMConfig):
        ...
        self.state_encoders = nn.ModuleDict({name: MLP(8, w, d) for name in self.FAMILIES})
        self.dynamics_processors = nn.ModuleDict({name: MLP(3 * d, w, d) for name in ("physical", "communication", "flow", "task_agent", "dag", "p2a", "p2c")})
    ...
    def route_actions(self, action: Mapping[str, torch.Tensor], state: Mapping[str, torch.Tensor], graph: Mapping[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        shapes = {
        ...
        def scatter(family: str, indices: torch.Tensor, values: torch.Tensor, allowed: torch.Tensor) -> None:
            for b in range(indices.shape[0]):
                for row in range(indices.shape[1]):
                    ...
                    routed[family][b, idx] += self.action_encoders[family](values[b, row])
        ...
        scatter("task", action["route_task_index"], action["route_values"], state["task_presence"])
        scatter("flow", action["route_flow_index"], action["route_values"], state["flow_presence"])
        return routed

    def _state_features(self, family: str, state: Mapping[str, torch.Tensor]) -> torch.Tensor:
        if family in {"physical", "agent"}:
            raw = torch.cat((state["position"], state["speed"][..., None], state["acceleration"][..., None], state["entity_type_index"][..., None].float(), state["entity_presence"][..., None].float(), state["uav_mask"][..., None].float()), -1)
        elif family == "communication":
            csi = state["csi"]
            raw = torch.cat((csi.mean(-1, keepdim=True), csi.std(-1, keepdim=True), state["comm_type_index"][..., None].float(), state["comm_validity"][..., None].float(), state["comm_presence"][..., None].float(), torch.zeros((*csi.shape[:2], 3), device=csi.device)), -1)
        elif family == "flow":
            raw = torch.cat((state["flow_total"][..., None], state["flow_remaining"][..., None], state["carrying_active"][..., None].float(), state["flow_presence"][..., None].float(), state["flow_source_index"][..., None].float(), state["flow_destination_index"][..., None].float(), torch.zeros((*state["flow_total"].shape, 2), device=state["flow_total"].device)), -1)
        else:
            raw = torch.cat((state["task_work_remaining"][..., None], state["task_progress"][..., None], state["task_lifecycle_index"][..., None].float(), state["task_presence"][..., None].float(), torch.zeros((*state["task_progress"].shape, 4), device=state["task_progress"].device)), -1)
    ...
    def dynamics_interaction(self, latent: Mapping[str, Any], graph: Mapping[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        phy_msg = self.dynamics_processors["physical"](torch.cat((src_phy, dst_phy, dst_phy - src_phy), -1)) * pv[..., None]
        ...
        comm_msg = self.dynamics_processors["communication"](torch.cat((gather(h["agent"], cs), gather(h["agent"], cd), h["communication"]), -1)) * comm_valid[..., None]
        ...
        flow_msg = self.dynamics_processors["flow"](torch.cat((gather(h["agent"], fs), gather(h["agent"], fd), h["flow"]), -1)) * flow_valid[..., None]
        ...
        ta_msg = self.dynamics_processors["task_agent"](torch.cat((gather(h["task"], ta_t), gather(h["agent"], ta_a), gather(h["agent"], ta_a) - gather(h["task"], ta_t)), -1)) * ta_valid[..., None]
        ...
        dag_msg = self.dynamics_processors["dag"](torch.cat((gather(h["task"], dag_src), gather(h["task"], dag_dst), gather(h["task"], dag_dst) - gather(h["task"], dag_src)), -1)) * dag_valid[..., None]
        dag_task = mean_to(dag_msg, dag_dst, dag_valid, h["task"].shape[1])

        p2a = self.dynamics_processors["p2a"](torch.cat((h["physical"], h["agent"], h["agent"] - h["physical"]), -1)) * graph["align_validity"][..., None]
        p2c = self.dynamics_processors["p2c"](torch.cat((gather(h["physical"], cs), gather(h["physical"], cd), h["communication"]), -1)) * graph["geo_comm_validity"][..., None]
        result = {
    ...
    def one_step(self, latent: Mapping[str, Any], state: Mapping[str, torch.Tensor], graph: Mapping[str, torch.Tensor], action: Mapping[str, torch.Tensor], *, prior_mode: str, service_mode: str, generator: torch.Generator | None) -> tuple[dict[str, Any], dict[str, torch.Tensor], dict[str, torch.Tensor], dict[str, Any]]:
        routed = self.route_actions(action, state, graph)
    ...
    def rebuild_graph(self, state: Mapping[str, torch.Tensor], previous: Mapping[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        graph["physical_relation_validity"] = state["entity_presence"][:, :, None] & state["entity_presence"][:, None, :] & (torch.linalg.vector_norm(delta, dim=-1) <= self.config.physical_radius_m)
        graph["comm_csi"] = state["csi"]
        graph["comm_csi_mask"] = state["csi_mask"]
        graph["comm_validity"] = state["comm_validity"]
        ...
        graph["task_agent_validity"] = state["task_agent_validity"]
    ...
    def synthetic_fixture(self, batch_size: int = 1) -> tuple[dict[str, Any], dict[str, torch.Tensor], dict[str, torch.Tensor], dict[str, torch.Tensor]]:
        ...
        state = {
            "entity_presence": torch.ones((b, ne), dtype=torch.bool), "entity_type_index": entity_type,
            ...
            "comm_presence": torch.ones((b, nc), dtype=torch.bool), "comm_validity": torch.ones((b, nc), dtype=torch.bool), "comm_type_index": torch.tensor([[COMM_WIRELESS, COMM_WIRELESS, COMM_WIRED]]).expand(b, -1).clone(),
            ...
            "flow_presence": torch.ones((b, nf), dtype=torch.bool), "flow_total": torch.full((b, nf), 10000.0), "flow_remaining": torch.full((b, nf), 5000.0), "flow_source_index": torch.zeros((b, nf), dtype=torch.long), "flow_destination_index": torch.tensor([[2, 3]]).expand(b, -1).clone(),
            "flow_identity_index": torch.arange(nf)[None].expand(b, -1).clone(), "flow_route_revision": torch.zeros((b, nf), dtype=torch.long),
        ...
        graph = {"physical_relation_features": torch.zeros((b, ne, ne, 4)), "physical_relation_validity": torch.ones((b, ne, ne), dtype=torch.bool), "comm_csi": state["csi"].clone(), "comm_csi_mask": state["csi_mask"].clone(), "comm_validity": state["comm_validity"].clone(), "comm_source_index": state["comm_source_index"].clone(), "comm_target_index": state["comm_target_index"].clone(), "flow_presence": state["flow_presence"].clone(), "flow_source_index": state["flow_source_index"].clone(), "flow_destination_index": state["flow_destination_index"].clone(), "task_presence": state["task_presence"].clone(), "task_agent_task_index": state["task_agent_task_index"].clone(), "task_agent_agent_index": state["task_agent_agent_index"].clone(), "task_agent_validity": state["task_agent_validity"].clone(), "dag_edges": torch.tensor([[[0, 1], [1, 2]]]).expand(b, -1, -1).clone(), "dag_validity": torch.ones((b, 2), dtype=torch.bool), "align_validity": state["entity_presence"].clone(), "geo_comm_validity": state["comm_wireless_mask"].clone()}
