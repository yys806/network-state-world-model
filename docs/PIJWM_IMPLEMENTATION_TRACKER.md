...
| Physical / Information 双图 | 03 | Step 4.1 mapping；Step 4.2A additive Sample/Tensor；Step 4.2C-B Ledger/Raw amendment；Step 4.2C-C Flow Sample/Tensor；旧 `formal_graph_ops_v1.py` / `formal_dual_graph_world_model_v1.py` | FLOW LEDGER + RAW + SAMPLE/TENSOR COMPLETE / FROZEN; GRAPH NOT_STARTED | STRUCTURAL_CHANGE | Input destination 来自 established route terminal，Return 来自 `return_destination_id`；真实两跳及跨时隙 trace 已证明单 FlowID/Epoch、固定 destination、稳定 Tensor slot 与 final-hop-only E2E；DepData runtime=0；真实 trace 未覆盖 Return multi-hop/same-destination partial-hop reroute/Epoch change | 研究者审阅后再授权 Definition 03 Graph Builder Contract；禁止自动建图 |
