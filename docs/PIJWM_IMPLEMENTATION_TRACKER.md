...
更新时间：2026-09-22。**Raw / 最小 Dataset-Tensor / Stateful Flow / Typed Dual-Graph Builder / Dual-Graph Encoder / STEP 4.4 Structured RSSM World Model Contract 均已完成并冻结；Definition 05 的 10 项 Researcher Decision 已由 STEP 5.0 冻结；STEP 5.1A-PATCH 与 STEP 5.1C-PATCH 已闭合 Future target、统一 identity lineage 和 dev_train normalization provenance**。STEP 5.1D 尚未开始；Training loop、GPU training 与 Planner 仍为 NOT STARTED。
...
| Definition 05 decision contract | 05；研究者 STEP 5.0 明确决定 | `contracts_PIJWM_DEFINITION_05_LOSS_TRAINING_EVALUATION_V1.md` | DECISION FROZEN / 5.1B-PATCH IN PROGRESS | STRUCTURAL_CHANGE | 10 项决策已冻结；Patch 正在修正逐 horizon teacher、真实 STEP 4.4 prior/decoder 接线、KL/metric 与 receipt；训练仍未开始 | 完成 Patch 验收后再决定 STEP 5.2 |
...
| STEP 5.1C unified identity/normalization lineage | 05 §1；4.2A/4.2C-B/C | `build_step5_1c_unified_development_bundle_v1.py`、unified bundle artifact | PATCH COMPLETE / CPU DEVELOPMENT ONLY | MINOR_MODIFICATION | 12 paired samples 的 Physical/Communication slot identity、10/74 capacity、no-prefix 和 dev_train-only stats 已机器闭合；4.3A/4.3B/4.4 尚未由 unified bundle 重建 | 另行授权 STEP 5.1D；不得自动进入 STEP 5.2 |
...
| loss / posterior / KL | 05 §1.2；STEP 5.0 决策 | `code/src/pi_jwm/step5_1b_posterior_loss_metric_v1.py` | PATCH CPU DEVELOPMENT EVIDENCE / NOT FROZEN | STRUCTURAL_CHANGE | Patch 保留 horizon、mask evidence、family-specific future teacher、真实 model prior/decoder、逐维 free bits 和完整 total-loss primitive；无训练循环 | Patch receipt 完成前不得进入 STEP 5.2 |
...
| prediction evaluation | 05 §3；STEP 5.0 决策 | `code/src/pi_jwm/step5_1b_posterior_loss_metric_v1.py` | PATCH CPU DEVELOPMENT EVIDENCE / NOT FROZEN | MINOR_MODIFICATION | Patch 提供逐 horizon raw-unit Motion component/xyz MAE/RMSE 与 CSI dB MAE/RMSE；未形成性能结果 | Patch receipt 完成前不得进入 STEP 5.2 |
...
STEP 4.3A、STEP 4.3B 与 STEP 4.4 均正式 COMPLETE / FROZEN；STEP 5.0 已冻结 Definition 05 决策和复用审计。STEP 5.1A-PATCH 已冻结 local one-step Motion、current physical slots 与 current model CSI slots。STEP 5.1B 原实现被研究者复核发现 temporal aggregation、独立 prior、fake h/identity loss、KL reduction、metric 和 receipt 语义问题；当前仅执行 STEP 5.1B-PATCH，尚未 COMPLETE/FROZEN。Patch 完成前不得训练、启动 GPU、访问 `locked_test` 或进入 STEP 5.2。
