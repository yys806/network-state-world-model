...
| Physical / Information 双图 | 03 | Step 4.1 mapping；Step 4.2A additive Sample/Tensor；Step 4.2B source receipt；Step 4.2C-A causal ledger feasibility；旧 `formal_graph_ops_v1.py` / `formal_dual_graph_world_model_v1.py` | FLOW LEDGER PARTIALLY FEASIBLE / GRAPH NOT_STARTED | STRUCTURAL_CHANGE | 真实 hop event 可部分追溯；跨 hop remaining、final delivery、reroute holder 仍需 event/observer hook；其他 dynamic resources 单独记录 | 研究者审阅缺失 event/hook 后再授权 Raw additive source；禁止直接建图 |
