...
| Definition 05 decision contract | 05；研究者 STEP 5.0 明确决定 | `contracts_PIJWM_DEFINITION_05_LOSS_TRAINING_EVALUATION_V1.md` | DECISION FROZEN / 5.1B PRIMITIVES COMPLETE | STRUCTURAL_CHANGE | 10 项决策已冻结；5.1B 已实现 target-only posterior、family loss、analytic KL、L_Val 与指标原语；训练仍未开始 | 研究者审阅后单独授权 STEP 5.2 |
...
| loss / posterior / KL | 05 §1.2；STEP 5.0 决策 | `code/src/pi_jwm/step5_1b_posterior_loss_metric_v1.py` | IMPLEMENTED / CPU DEVELOPMENT EVIDENCE | STRUCTURAL_CHANGE | v1 只用 Motion/CSI mask-MSE + family analytic KL；overshooting OFF；target encoder、mask、free-bits、raw→normalized bridge 已实现；无训练循环 | STEP 5.2 training contract 仍未授权 |
...
| prediction evaluation | 05 §3；STEP 5.0 决策 | `code/src/pi_jwm/step5_1b_posterior_loss_metric_v1.py` | IMPLEMENTED / CPU DEVELOPMENT EVIDENCE | MINOR_MODIFICATION | v1 提供逐 horizon Motion component/xyz MAE/RMSE 与 CSI dB MAE/RMSE；NLL非核心；未形成性能结果 | STEP 5.2 prior-only validation runner 仍未授权 |
...
STEP 4.3A、STEP 4.3B 与 STEP 4.4 均正式 COMPLETE / FROZEN；STEP 5.0 已冻结 Definition 05 决策和复用审计。STEP 5.1A-PATCH 已冻结 local one-step Motion、current physical slots 与 current model CSI slots。STEP 5.1B 已完成 CPU 原语实现与 receipt：posterior 只接收 target，prior 不接收 target，loss/KL/metric 有 finite、梯度和 reload 证据；仍不是正式 Dataset、训练或性能结果。唯一下一动作建议是研究者审阅后另行授权 STEP 5.2；不得自动执行、不得启动 GPU 或访问 `locked_test`。
