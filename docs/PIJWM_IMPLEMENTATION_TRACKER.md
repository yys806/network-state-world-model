...
更新时间：2026-09-22。**Raw / 最小 Dataset-Tensor / Stateful Flow / Typed Dual-Graph Builder / Dual-Graph Encoder / STEP 4.4 Structured RSSM World Model Contract 均已完成并冻结；Definition 05 的 10 项 Researcher Decision 已由 STEP 5.0 冻结；STEP 5.1A-PATCH、STEP 5.1C-PATCH 与 STEP 5.1D 已闭合 Future target、统一 identity lineage、dev_train normalization provenance 和 paired model-chain CPU integration**。Training loop、GPU training 与 Planner 仍为 NOT STARTED。
...
| Definition 05 decision contract | 05；研究者 STEP 5.0 明确决定 | `contracts_PIJWM_DEFINITION_05_LOSS_TRAINING_EVALUATION_V1.md` | DECISION FROZEN / 5.1D CLOSED | STRUCTURAL_CHANGE | 10 项决策已冻结；5.1B primitives 与 5.1D paired CPU evidence 已闭合；训练仍未开始 | 研究者另行授权 STEP 5.2 |
...
| STEP 5.1D unified model-chain paired acceptance | 05 §1；4.3A/4.3B/4.4 | `build_step5_1d_unified_model_chain_v1.py`、`STEP_05_1D_UNIFIED_MODEL_CHAIN_PAIRED_ACCEPTANCE.md`、tracked evidence artifact | COMPLETE / FROZEN FOR CPU DEVELOPMENT INTEGRATION | STRUCTURAL_CHANGE | 47/47 checks、12/12 pairing、exact upstream train lineage、runtime prior isolation、actual prior/posterior/decoder、gradient 和 deterministic rebuild 已通过；模型仍 untrained；Route/Comp non-empty coverage=0 | 研究者另行授权 STEP 5.2；不得自动执行 |
...
| loss / posterior / KL | 05 §1.2；STEP 5.0 决策 | `code/src/pi_jwm/step5_1b_posterior_loss_metric_v1.py` + paired 5.1D receipt | COMPLETE / FROZEN FOR CPU DEVELOPMENT PRIMITIVES | STRUCTURAL_CHANGE | paired integration 已通过；无训练循环、无性能结论 | STEP 5.2 需单独授权 |
...
STEP 4.3A、STEP 4.3B 与 STEP 4.4 均正式 COMPLETE / FROZEN；STEP 5.0 已冻结 Definition 05 决策和复用审计。STEP 5.1A-PATCH 已冻结 local one-step Motion、current physical slots 与 current model CSI slots。STEP 5.1B-PATCH 与 STEP 5.1D-PATCH 已 COMPLETE/FROZEN FOR CPU DEVELOPMENT PRIMITIVES + PAIRED INTEGRATION；训练、GPU、`locked_test` 与 STEP 5.2 仍未开始。
