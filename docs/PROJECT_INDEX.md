...
> 生成于 2026-09-08，状态更新于 2026-09-09。当前正式训练已停止；`code/artifacts/` 仍是受保护证据区，不做移动、覆盖或清理。

## 1. 进入项目的最短路径

按下面顺序阅读，能够先知道规则、当前状态和证据边界，再进入具体实现：

1. [`AGENTS.md`](../AGENTS.md)：永久治理规则、证据口径和安全边界。
2. [`task_plan.md`](../task_plan.md)：当前任务、停止门和唯一下一动作。
3. [`记录/文件树与证据分层_20260826.md`](../记录/文件树与证据分层_20260826.md)：文件职责和证据等级。
4. [`记录/本地计划表.md`](../记录/本地计划表.md)：项目粗粒度路线和阶段边界。
5. [`记录/PIJWM主文档.md`](../记录/PIJWM主文档.md)：理论、数据、方法和评价定义。
6. [`记录/8.12之后推进.md`](../记录/8.12之后推进.md)：最新推进、失败和阻塞。
7. [`docs/RESEARCH_STATUS.md`](RESEARCH_STATUS.md)：面向人和 AI 的当前研究状态摘要。
8. `code/artifacts/` 中与问题直接对应的 manifest、audit、runtime 和原始结果：最终判断必须回到这里核实。
...
| GPU 训练 | `code/scripts/run_formal_dual_graph_gpu_train_v1.py` | 两阶段 base/RSSM 训练和 checkpoint 选择 |
...
| 旧方法为什么弃用 | `记录/研究进展/`、历史 audit | 失败门和诊断 artifact |
