# STEP 3.1 — Model-ready Sample & Tensor Contract Freeze
...
- Step 2 Raw Contract 与最终通信证据：`docs/contracts_PIJWM_RAW_SINGLE_DECISION_STEP_CONTRACT_V1.md`、`code/artifacts/protocols/pi_jwm_communication_outcome_semantics_v1_20260919/`。
- 研究笔记目录未修改。

## Initial State

旧 formal dataset/window/tensor 已有 ID/index、mask、DAG endpoint、trajectory split 和 masked normalization 机制，但旧 `flow_state` 语义不能直接代表新定义 Flow/Data；Step 2 Raw artifact 的 DAG dependency rows 也没有被真实 observer 独立捕获。
...
- 固定最小 `H=2, L=2`：anchor frame 2 的 History 是 frame 0–1；Future Action/Target 是 frame 2–3；当前 action 不进入 History。
...
- Flow/Data 依据真实 transfer event 的 task/transport/source/destination 生成 hop-level flow ID。DAG 没有可靠 Raw 来源，记录 explicit missing gap，不从旧 tensor 猜。
...
- History frame `0,1`，Future Action/Target frame `2,3`。
...
- DAG 状态为 `observed_mask=false` 且原因 `RAW_DAG_DEPENDENCY_SOURCE_NOT_CAPTURED`，表示证据缺口，不表示真实 DAG 为空。
...
- Raw observer 尚未提供独立 DAG dependency rows；当前只能记录 missing gap。
