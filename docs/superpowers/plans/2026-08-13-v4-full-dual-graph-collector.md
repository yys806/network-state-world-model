...
**Execution status (2026-08-14):** Tasks 1--9 are implemented. Task 10 self-review found that the first canonical manifest hashed only eight newly added sources and therefore did not satisfy the frozen direct/transitive source, test, design, and configuration closure. The remediation expands the canonical matrix to 116 files and adds a RED-then-GREEN regression. Independent recomputation then exposed worktree-prefixed keys caused by resolving the AirFogSim junction; a second RED-then-GREEN correction preserves project-logical relative keys. Both superseded bundles must remain diagnostic archives, and the canonical bundle must be regenerated after this tracked plan/design state is committed. Completion is determined by the regenerated manifest and fresh verifier output. Integration into `main` remains pending explicit user direction. The detailed checkboxes below preserve the original execution recipe rather than serving as the authoritative status ledger.
