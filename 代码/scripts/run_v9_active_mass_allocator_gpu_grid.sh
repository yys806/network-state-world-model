...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_active_mass_allocator_20260617"
mkdir -p "$OUT"
...
run_one m0_hurdle_soft_baseline \
  --seed 20260614 \
  --model-rate-output-mode hurdle_soft \
  --rate-output-mode main \
  --active-mass-loss-weight 0.0
...
run_one m3_mass_w10 \
  ...
  --model-rate-output-mode hurdle_mass \
  --rate-output-mode active_mass_alloc \
  --active-mass-loss-weight 1.0

run_one m4_mass_w03_seed20260617 \
  --seed 20260617 \
  --model-rate-output-mode hurdle_mass \
  --rate-output-mode active_mass_alloc \
  --active-mass-loss-weight 0.3
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_active_mass_allocator_20260617")
anchor = {
...
rows = [anchor]
names = [
    "m0_hurdle_soft_baseline",
    "m1_mass_w01",
    "m2_mass_w03",
    "m3_mass_w10",
    "m4_mass_w03_seed20260617",
]
for name in names:
    ...
    mass_losses = [row.get("active_mass_total_loss") for row in train_rows if row.get("active_mass_total_loss") is not None]
    row = {
        "experiment": name,
        "seed": cfg.get("seed"),
        ...
        "avg_train_active_mass_total_loss": sum(mass_losses) / len(mass_losses) if mass_losses else None,
...
baseline = next((row for row in rows if row.get("experiment") == "m0_hurdle_soft_baseline" and "error" not in row), anchor)
for row in rows[1:]:
    if "error" in row:
        continue
    ...
    row["active_rate_delta_vs_m0"] = row["active_rate_rmse"] - baseline["active_rate_rmse"]
    row["link_rate_delta_vs_m0"] = row["link_rate_rmse"] - baseline["link_rate_rmse"]
    row["f1_delta_vs_m0"] = row["f1"] - baseline["f1"]
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "active_mass_allocator_comparison.csv").open("w", newline="") as f:
...
with (out / "active_mass_allocator_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 Active-Mass Allocator GPU Grid\n\n")
    f.write("Candidate: decouple active-rate amplitude by predicting sample-step active-rate mass and allocating it across candidate edges. Reportable gate: active-rate < C8a, link RMSE <= 90, F1 >= C8a.\n\n")
    f.write("C8a anchor: active-rate=292.830303, F1=0.027108, link=89.141847.\n\n")
    f.write("| Experiment | seed | model mode | rate output | mass w | avg mass loss | best epoch | P | R | F1 | Active RMSE | Pos-active RMSE | Mass-active RMSE | Link RMSE | Node RMSE | Task RMSE | d active C8a | d link C8a | d F1 C8a | d active m0 | d link m0 | d F1 m0 | reportable |\n")
    f.write("|---|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|\n")
    for row in rows:
        f.write("| " + " | ".join([
            ...
            fmt(row.get("avg_train_active_mass_total_loss")),
            ...
            fmt(row.get("active_rate_delta_vs_m0")),
            fmt(row.get("link_rate_delta_vs_m0")),
            fmt(row.get("f1_delta_vs_m0")),
            fmt(row.get("meets_reportable_gate")),
        ]) + " |\n")
print(out / "active_mass_allocator_comparison.csv")
PY
