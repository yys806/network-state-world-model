...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_gate_aware_selection_20260617"
mkdir -p "$OUT"
...
run_one s0_original_gate --best-metric val_precision_constrained_composite --eval-hurdle-gate-power 1.0
run_one s1_gate_power05_active --best-metric val_active_rate_rmse --eval-hurdle-gate-power 0.5
run_one s2_gate_power075_active --best-metric val_active_rate_rmse --eval-hurdle-gate-power 0.75
run_one s3_gate_power05_composite --best-metric val_precision_constrained_composite --eval-hurdle-gate-power 0.5
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_gate_aware_selection_20260617")
anchor = {
...
rows = [anchor]
for name in ["s0_original_gate", "s1_gate_power05_active", "s2_gate_power075_active", "s3_gate_power05_composite"]:
    ...
    row = {
        "experiment": name,
        "eval_hurdle_gate_temperature": cfg.get("eval_hurdle_gate_temperature"),
        "eval_hurdle_gate_power": cfg.get("eval_hurdle_gate_power"),
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "gate_aware_selection_comparison.csv").open("w", newline="") as f:
...
with (out / "gate_aware_selection_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 Gate-Aware Selection GPU Grid\n\n")
    f.write("| Experiment | gate power | best metric | best epoch | P | R | F1 | Active-rate RMSE | Positive-rate active RMSE | Link RMSE | Node RMSE | Task RMSE | d active | d positive | d link | d F1 |\n")
    f.write("|---|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        f.write("| " + " | ".join([
            fmt(row.get("experiment")),
            fmt(row.get("eval_hurdle_gate_power")),
...
print(out / "gate_aware_selection_comparison.csv")
PY
