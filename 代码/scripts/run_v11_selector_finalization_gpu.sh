...
run_logged "01_full_train_calibration_validation_labels" \
  "${PYTHON}" scripts/run_v11_selector_candidate_labels.py \
  --splits train calibration validation \
  "${COMMON_LABEL_ARGS[@]}"

run_logged "02_validation_selected_selector_training" \
...
run_logged "03_validation_feature_group_ablation" \
...
run_logged "04_matched_test_labels_after_freeze" \
  "${PYTHON}" scripts/run_v11_selector_candidate_labels.py \
  --splits matched_test \
  --frozen-config-manifest "${FROZEN_MANIFEST}" \
  "${COMMON_LABEL_ARGS[@]}"

run_logged "05_one_shot_matched_test_evaluation" \
...
run_logged "06_freeze_result_bundle" \
