...
OUT="$ROOT/artifacts/experiments/pi_jwm_v9_high_rate_weight_20260617"
mkdir -p "$OUT"
...
run_one h0_high_w1_baseline --high-rate-weight 1.0
run_one h1_high_w2 --high-rate-weight 2.0
run_one h2_high_w4 --high-rate-weight 4.0
run_one h3_high_w8 --high-rate-weight 8.0
...
out = Path("/root/autodl-tmp/pi_jwm_gpu_m5/artifacts/experiments/pi_jwm_v9_high_rate_weight_20260617")
anchor = {
...
rows = [anchor]
names = ["h0_high_w1_baseline", "h1_high_w2", "h2_high_w4", "h3_high_w8"]
for name in names:
    ...
    train_rows = [epoch.get("train", {}) for epoch in summary.get("history", [])]
    high_rate_counts = [row.get("high_rate_count") for row in train_rows if row.get("high_rate_count") is not None]
    row = {
        "experiment": name,
        "high_rate_weight": cfg.get("high_rate_weight"),
        "high_rate_threshold": cfg.get("high_rate_threshold"),
        "avg_train_high_rate_count": sum(high_rate_counts) / len(high_rate_counts) if high_rate_counts else None,
...
baseline = next((row for row in rows if row.get("experiment") == "h0_high_w1_baseline" and "error" not in row), anchor)
for row in rows[1:]:
    if "error" in row:
        continue
    ...
    row["active_rate_delta_vs_h0"] = row["active_rate_rmse"] - baseline["active_rate_rmse"]
    row["positive_rate_active_delta_vs_h0"] = row["positive_rate_active_rmse"] - baseline["positive_rate_active_rmse"]
    row["link_rate_delta_vs_h0"] = row["link_rate_rmse"] - baseline["link_rate_rmse"]
    row["f1_delta_vs_h0"] = row["f1"] - baseline["f1"]
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
with (out / "high_rate_weight_comparison.csv").open("w", newline="") as f:
...
with (out / "high_rate_weight_comparison.md").open("w") as f:
    f.write("# PI-JWM v9 High-Rate Positive Weight GPU Grid\n\n")
    f.write("| Experiment | high w | threshold | avg high cnt | best epoch | P | R | F1 | Active-rate RMSE | Positive-rate active RMSE | Link RMSE | Node RMSE | Task RMSE | d active C8a | d positive C8a | d link C8a | d F1 C8a | d active h0 | d link h0 | d F1 h0 |\n")
    f.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for row in rows:
        f.write("| " + " | ".join([
            fmt(row.get("experiment")),
            fmt(row.get("high_rate_weight")),
            fmt(row.get("high_rate_threshold")),
            fmt(row.get("avg_train_high_rate_count")),
            ...
            fmt(row.get("active_rate_delta_vs_h0")),
            fmt(row.get("link_rate_delta_vs_h0")),
            fmt(row.get("f1_delta_vs_h0")),
        ]) + " |\n")
print(out / "high_rate_weight_comparison.csv")
PY
