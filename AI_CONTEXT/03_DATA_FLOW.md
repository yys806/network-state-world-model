...
- STEP 5.1A：`code/src/pi_jwm/step5_1a_motion_csi_target_contract_v1.py` 从 STEP 4.2A Sample 与真实 raw `outcome.channel_rows[].channel_attenuation_db` 构造 additive Future Target。Vehicle Motion 为 `[delta_x, delta_y, delta_z, next_speed]`；delta 使用冻结 position std（不减 position mean），speed/CSI 使用冻结 train-only mean/std。CSI 只覆盖 current support relation slot，按 relation identity 与 RB index 对齐；wired、future 缺失和 invalid relation 保留 identity 但 mask=false。NPZ tensor contract 保留 normalization parameters、stats digest、entity/relation identity 和 endpoint/RB metadata。12 个 non-locked development samples 的 receipt `passed=true`；这不是正式 Dataset，当前模型尚未读取 target。
- STEP 5.1A 保持 target 与 History/Input namespace 隔离；unsupported/unresolved/fixed-support-blocked 只进入 side metadata，不删除 window，也不改 Motion/CSI masks。posterior/loss/metric/training 尚未实现，prior input 保持不变。
