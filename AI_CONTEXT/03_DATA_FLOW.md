...
- STEP 5.2：`step5_2_training_loop_v1.py` 使用同一 12-sample unified lineage（`dev_train=8`、`dev_validation=4`）。Stage 1 将 Motion/CSI target 分别编码到对应 future posterior，并用 posterior mean 做训练期预测与 KL；Stage 2 和 validation 通过 `initialize_latent(posterior_mode="prior")` 做 prior-only recursive rollout，下一步状态来自上一步预测状态和确定性规则反馈。目标只用于 Motion/CSI loss 与 training-only KL teacher；validation 使用 `model.eval()+no_grad()`，不调用 posterior。该路径仅完成少量 CPU optimizer smoke、prior-only validation 与 checkpoint/resume。
