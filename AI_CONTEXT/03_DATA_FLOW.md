...
- Step 3.1R 最小样本：History 为 `[t-H+1,t]` 的 `H=2` 帧，Future Action/Target 为 `[t,t+L-1]` 的 `L=2` 帧；input index 在整个 History 固定，target-side physical/task/flow namespace 单独表达未来对象。真实 DAG rows 来自 observer，future-only endpoint 只在 Raw internal metadata，不进入 input Static。
