...
- STEP 3.3 当前 CPU collation 入口为 `code/src/pi_jwm/step3_3_model_input_tensor_v1.py`；12 个 Step 3.2 sample 生成 fixed-shape arrays。stable index 不重新编号，padding index=-1、numeric=0，presence/mask 独立；target-only 使用独立 target capacities。该 tensor 尚未被双图或模型读取。
