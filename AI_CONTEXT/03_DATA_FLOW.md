...
- STEP 4.2C-C：`code/src/pi_jwm/step4_2c_c_flow_sample_tensor_v1.py` 消费 C-B Raw 的 `logical_flow_rows/carrying_rows`，建立独立 History `logical_flow` union 与 target namespace。Logical Flow 保存 FlowID/Epoch/destination/total/E2E/presence/status；Carrying 保存 holder/hop/route/progress；completed/superseded known row 与 padding 分开。仅 `total_data/e2e_delivered/e2e_remaining/hop_progress/hop_remaining` 在 `dev_train` valid mask 上标准化，单位为 AirFogSim data-unit；categorical/reference/Epoch/RouteRevision/presence 不标准化。Tensor artifact 是 CPU/non-locked development evidence，不是 formal Dataset。
