...
- STEP 5.0 target audit：future position raw value 已在 STEP 4.2A sample，但没有 target normalization/tensor；future speed 已在 `target_entity_features`；future per-RB CSI target 目前完全缺失。STEP 5.1 必须按现有 train-only stats做 additive target tensor，不得 refit；posterior whitelist 只接 Motion/CSI target，prior input保持不变；unsupported structure 只产生 component side masks/counts。
